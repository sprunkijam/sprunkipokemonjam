#!/usr/bin/env python3
"""Add Fun Bot Phase 1/2 art, Icons 04 face zooms, sticky Phase 2."""
...
ART_P1 = ROOT / "art" / "funbot-phase1.png"
ART_P2 = ROOT / "art" / "funbot-phase2.png"
PREV = ROOT / "art" / "_preview"
OUT_CHECK = Path("/workspace/funbot-p1-check.png")

SPRITE = "Fun Bot"
ICON_SLOT = "04"
# Face-centered crop on phase1 (810x400): cyan rim + eyes + mouth
ICON_LEFT, ICON_TOP, ICON_SIDE = 279, 50, 240
...
def ensure_b3_costume(target: dict) -> None:
    ...
    if "b3" in names:
        ...
        return
    # Prefer cloning anim??? (horror) as template; fallback idle
    template = next(
...
def fix_fake04_refs(icons: dict) -> int:
    """Retarget any fake04 hard-switches to real 04-* PNG costumes."""
    blocks = icons["blocks"]
    fixed = 0
    for bid, b in blocks.items():
        if not isinstance(b, dict):
        ...
        if not costume:
            continue
        name = costume[0]
        if "fake" in name.lower() and name.startswith("04"):
            # 04-a-fake04 → 04-a etc.
            base = name.split("-fake")[0]
            if not base.startswith("04"):
                base = "04-a"
...
def main() -> None:
    ...
    # checker preview for p1 (refresh)
    ...
    print(f"3) Fun Bot size={target.get('size')} (wide canvas → ~40)")
    target["size"] = 40.0
    ...
    fixed = fix_fake04_refs(icons)
    print(f"  fake04 retargets: {fixed}")
    ...
    # Also replace any 04-*-fake* costume assets if present
    for c in icons["costumes"]:
        if c["name"].startswith("04") and "fake" in c["name"].lower():
            # map to a/b/c
            suffix = "a"
            if "-b" in c["name"]:
                suffix = "b"
    ...
    fb = next(t for t in data2["targets"] if t["name"] == SPRITE)
    print("VERIFY Fun Bot size", fb.get("size"))
    print("VERIFY costumes:")
    for c in fb["costumes"]:
    ...
    for c in ic["costumes"]:
        if c["name"].startswith("04"):
            print(f"  Icons {c['name']}: {c.get('md5ext')} {c.get('dataFormat')}")
