#!/usr/bin/env python3
"""Redo all custom sprites from ORIGINAL uploads with keep-mask bg removal (black outlines preserved).
...
CHARS = {
    ...
    "simon": {
        ...
        "mode": "copy_originals",
        "preserve_size": 55.0,
        "p1_centers": (114.5, 200.0),
        "p2_centers": (136.0, 200.0),
    },
    "pinki": {
        "sprite": "Pink (Pinki)",
        "slot": "18",
    ...
    "mrblack": {
        "sprite": "Black?",
        "slot": "20",
        "icon": {"side": 200, "top": 28, "left_bias": 0},
    ...
    "mr-tree": {
        ...
        "slot": "13",
        "icon": {"side": 220, "top": 10, "left_bias": 0},
        "p1": "107323c58dc5e2057170288298a83bb437e96a6ae06c707221bb16f6cecd38d2.png",
        "p2": "8a696f7b40bbfe16b7e3a5752f0347b7eda4eac170e2c36596cfc3ff566affc3.png",
        "mode": "edge",
        "ensure_b3": True,
        "sticky": True,
    },
    "jevin": {
        "sprite": "Blue (Jevin)",
        "slot": "19",
        "icon": {"side": 220, "top": 20, "left_bias": 0},
        "p1": "45bccc76c65dbc72cd0657db0a52336ddd58c06cfcbfc31245f3af61662197aa.png",
        "p2": "e39930fa457f74575a749b31c09a7136b11be70d3412fe646d6a07a8af52eb81.png",
        "mode": "edge",
        "ensure_b3": True,
        "sticky": True,
    },
    ...
    for char, meta in CHARS.items():
        ...
        for phase, key in (("phase1", "p1"), ("phase2", "p2")):
            ...
            if mode == "copy_originals":
                ...
                att_name = meta[key]
                if not att_name:
                    notes[f"{char}-{phase}"] = "skipped (no attachment)"
                    print(f"SKIP {dest.name} (no attachment)")
                    continue
                src = ATT / att_name
                if not src.exists():
...
def reembed(icon_pngs: dict[str, bytes]) -> None:
    ...
    assert float(simon2.get("size")) == 55.0, simon2.get("size")
