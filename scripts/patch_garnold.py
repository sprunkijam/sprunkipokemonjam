#!/usr/bin/env python3
"""Add Gold (Garnold) Phase 1/2 art, Icons 08 face zooms, sticky Phase 2.

Wide 4-leg Metagross-style canvas → size ~45 (between Fun Bot/Clukr ~40 and Gray ~50).
"""
...
ART_P1 = ROOT / "art" / "garnold-phase1.png"
ART_P2 = ROOT / "art" / "garnold-phase2.png"
PREV = ROOT / "art" / "_preview"
OUT_CHECK = Path("/workspace/garnold-p1-check.png")

SPRITE = "Gold (Garnold)"
ICON_SLOT = "08"
# Face/visor crop on phase1 (489x400): yellow stripe + black visor + yellow bar
ICON_LEFT, ICON_TOP, ICON_SIDE = 160, 35, 165
POLO_ID = "OCOFFrF7vAMtV(f.kUS7"  # Polo Im On RN (Character == 8)
GARNOLD_SIZE = 45.0
# Start Polo drop: looks_goforwardbackwardlayers cB → looks_show nu
LAYER_ID = "cB"
SHOW_ID = "nu"
...
def ensure_drop_size(target: dict, size: float) -> None:
    """Insert/update looks_setsizeto on Start Polo drop (cB → nu)."""
    ...
    if layer.get("next") == SHOW_ID:
        ...
        if mb and mb.get("opcode") == "looks_setsizeto":
            ...
            raise RuntimeError(
                f"{LAYER_ID}.next expected {SHOW_ID} or setsizeto, got {mid}"
            )
...
def main() -> None:
    ...
    print("1) Face icons from Phase 1 (visor)")
    ...
    checker(Image.open(ART_P2).convert("RGBA")).save(PREV / "garnold-phase2-checker.png")
    ...
    print(f"3) Garnold size={target.get('size')} → {GARNOLD_SIZE} (wide 4-leg)")
    ensure_drop_size(target, GARNOLD_SIZE)

    for peer in ("Silver (Clukr)", "Fun Bot", "Gray (Gray)"):
        p = next(t for t in data["targets"] if t["name"] == peer)
        print(f"   {peer} size left at {p.get('size')}")
    ...
    print("5) Icons 08-* (Create slot; no fake* hard-switches)")
    ...
    for bid, b in blocks.items():
        ...
        if not costume:
            continue
        name = costume[0]
        if isinstance(name, str) and "fake" in name.lower() and name.startswith("08"):
            base = name.split("-fake")[0]
            print(f"  retarget looks_costume {bid}: {name} -> {base}")
            fields["COSTUME"] = [base, None]
            fixed += 1
    print(f"  fake08 retargets: {fixed}")
    ...
    g = next(t for t in data2["targets"] if t["name"] == SPRITE)
    print("VERIFY Garnold size", g.get("size"))
    p1_md = p2_md = None
    for c in g["costumes"]:
        print(
    ...
    for peer in ("Silver (Clukr)", "Fun Bot", "Gray (Gray)"):
        p = next(t for t in data2["targets"] if t["name"] == peer)
        print(f"VERIFY {peer} size unchanged", p.get("size"))
    print("DONE")
