#!/usr/bin/env python3
"""Add Gray (Gray) Phase 1/2 art, Icons 06 face zooms, sticky Phase 2.

Wide DJ-deck canvas → size ~50 (between Fun Bot/Clukr ~40 and peers ~55).
"""
...
ART_P1 = ROOT / "art" / "gray-phase1.png"
ART_P2 = ROOT / "art" / "gray-phase2.png"
PREV = ROOT / "art" / "_preview"
OUT_CHECK = Path("/workspace/gray-p1-check.png")

SPRITE = "Gray (Gray)"
ICON_SLOT = "06"
# Face crop on phase1 (~378x400): eyes + pink mouth (minimal deck lip)
ICON_LEFT, ICON_TOP, ICON_SIDE = 137, 95, 125
POLO_ID = "3iC}*l:)ZbtH{v#-ST5j"  # Polo Im On RN (Character == 6)
GRAY_SIZE = 50.0
# Start Polo drop: looks_goforwardbackwardlayers cd → looks_show m*
LAYER_ID = "cd"
SHOW_ID = "m*"
...
def ensure_drop_size(target: dict, size: float) -> None:
    """Insert/update looks_setsizeto on Start Polo drop (cd → m*)."""
...
def main() -> None:
    ...
    checker(Image.open(ART_P2).convert("RGBA")).save(PREV / "gray-phase2-checker.png")
    ...
    print(f"3) Gray size={target.get('size')} → {GRAY_SIZE} (wide DJ deck)")
    ensure_drop_size(target, GRAY_SIZE)

    # Do not clobber Clukr / Fun Bot
    for peer in ("Silver (Clukr)", "Fun Bot"):
    ...
    print("5) Icons 06-* (Create slot; no fake* hard-switches)")
    ...
    for bid, b in blocks.items():
        ...
        if not costume:
            continue
        name = costume[0]
        if isinstance(name, str) and "fake" in name.lower() and name.startswith("06"):
            base = name.split("-fake")[0]
            print(f"  retarget looks_costume {bid}: {name} -> {base}")
            fields["COSTUME"] = [base, None]
            fixed += 1
    print(f"  fake06 retargets: {fixed}")
    ...
    gray = next(t for t in data2["targets"] if t["name"] == SPRITE)
    print("VERIFY Gray size", gray.get("size"))
    p1_md = p2_md = None
    for c in gray["costumes"]:
        print(
    ...
    sizes = [
        (bid, b["inputs"]["SIZE"])
        for bid, b in gray["blocks"].items()
        if isinstance(b, dict) and b.get("opcode") == "looks_setsizeto"
    ]
