#!/usr/bin/env python3
"""Add Tan (Tunner) Phase 1/2 art, Icons 15 face zooms, sticky Phase 2."""
...
ART_P1 = ROOT / "art" / "tunner-phase1.png"
ART_P2 = ROOT / "art" / "tunner-phase2.png"
PREV = ROOT / "art" / "_preview"
OUT_CHECK = Path("/workspace/tunner-p1-check.png")

SPRITE = "Tan (Tunner)"
ICON_SLOT = "15"
# Face-centered crop on phase1 (289x400): skull + cowboy hat + headphones
ICON_LEFT, ICON_TOP, ICON_SIDE = 25, 0, 240
...
def fix_fake15_refs(icons: dict) -> int:
    blocks = icons["blocks"]
    fixed = 0
    for bid, b in blocks.items():
        if not isinstance(b, dict):
        ...
        if not costume:
            continue
        name = costume[0]
        if "fake" in name.lower() and name.startswith("15"):
            base = name.split("-fake")[0]
            if not base.startswith("15"):
                base = "15-a"
...
def main() -> None:
    ...
    print(f"3) Tunner size={target.get('size')} → 55")
    target["size"] = 55.0
    ...
    fixed = fix_fake15_refs(icons)
    print(f"  fake15 retargets: {fixed}")
    ...
    for c in icons["costumes"]:
        if c["name"].startswith("15") and "fake" in c["name"].lower():
            suffix = "a"
            if "-b" in c["name"]:
                suffix = "b"
            elif "-c" in c["name"]:
    ...
    print("VERIFY Tunner size", wenda.get("size"))
